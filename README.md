...
- PDF export prunes each page's image resource dictionary so extractors see only
  the background image actually drawn on that page, even when jsPDF internally
  starts from shared image resources.
